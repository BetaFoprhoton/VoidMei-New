...
import org.matrixsukhoi.voidmei.prog.FMParts;
...
import static org.matrixsukhoi.voidmei.VoidMeiMainKt.*;
...
public class Blkx {
	...
	public FMParts NoFlapsWing;
	public FMParts NoFlapsWing_V50;
	public FMParts NoFlapsWing_V100;
	
	public FMParts FullFlapsWing;
	public FMParts FullFlapsWing_V50;
	public FMParts FullFlapsWing_V100;
	
	public Boolean isVWing;
	
	public FMParts Fuselage;
	public FMParts Fin;
	public FMParts Stab;
	...
	public void getPartsFm(String string, FMParts parts) {
		parts.name = string;
		parts.cdMin = getdouble(string + ".CdMin");
		parts.cl0 = getdouble(string + ".Cl0");
		parts.clCritHigh = getdouble(string + ".ClCritHigh");
		parts.clCritLow = getdouble(string + ".ClCritLow");

		parts.clAfterCrit = getdouble(string + ".ClAfterCrit");
		parts.lineClCoeff = getdouble(string + ".lineClCoeff");

		parts.aoACritHigh = getdouble(string + ".alphaCritHigh");
		parts.aoACritLow = getdouble(string + ".alphaCritLow");

	}

	public boolean getEngineLoad(engineLoad[] eL, int loadIndex) {
		String c = "Load" + loadIndex;
		// VoidMeiMain.Companion.debugPrint(c);
		eL[loadIndex].WaterLimit = getdouble(c + ".WaterTemperature");
		// VoidMeiMain.Companion.debugPrint(eL[loadIndex].WaterLimit);
	...
	public String WritePartsFm(String s, FMParts p) {
...
//		s = s.concat("临界攻角升力系数:[" + p.ClCritLow + ", " + p.ClCritHigh + "]" + "\n");
		//
		// VoidMeiMain.Companion.debugPrint("------fm器件 "+p.name+"------");
		// VoidMeiMain.Companion.debugPrint("零升阻力系数:"+p.CdMin);
		// VoidMeiMain.Companion.debugPrint("零攻角升力:"+p.Cl0);
		// VoidMeiMain.Companion.debugPrint("临界攻角:["+p.AoACritLow+","+p.AoACritHigh+"]");
		// VoidMeiMain.Companion.debugPrint("临界攻角升力系数:["+p.ClCritLow+","+p.ClCritHigh+"]");
		return s;
	}
		...
		if (!getone(c).equals("null")) {
			...
			for (int i = 0; i < num; i++) {
				try {
					...
					getLogger().error("getDouble error {}", c);
					return null;
				}
			}
		}
...
//				return i;
	...
	private FMParts FullFlapsWingS;
	private FMParts NoFlapsWingS;
	...
	public void initEngineLoad(){
		...
		engLoad = new engineLoad[VoidMeiMain.maxEngLoad];
		for (int i = 0; i < VoidMeiMain.maxEngLoad; i++) {
	...
	public void getload() {
		...
		// VoidMeiMain.Companion.debugPrint(getone("Load0.WaterTemperature"));
		isJet = false;

		// 读取推力高度
		engineNum = 1;
//		VoidMeiMain.Companion.debugPrint(getone("EngineType0.Main.Type"));
		String hdrString = "EngineType0.";
		String res = getone("EngineType0.Main.Type");
//		VoidMeiMain.Companion.debugPrint(res);
		if (res.equals("\"Jet\"")) {
			// 判断喷气
			isJet = true;
//			VoidMeiMain.Companion.debugPrint(getone("Engine"+engineNum));
        ...
        VoidMeiMainKt.getLogger().info("engineNum {}", engineNum);
...
//			thrMax0 = getdouble(hdrString + "Main.Thrust");
			...
			VoidMeiMainKt.getLogger().info("engineType: jet, afterburner coeff" + aftbCoff);
			altThrNum = 0;
			...
			for (int i = 0; i < 30; i++, altThrNum++) {
				...
				// VoidMeiMain.Companion.debugPrint(altitudeThr[i]);
			}
			// 读取推力速度
			velThrNum = 0;
			...
			for (int i = 0; i < altThrNum; i++) {
				...
				for (int j = 0; j < velThrNum; j++) {
					...
					VoidMeiMainKt.getLogger().info(String.format("[%.0f]%.0f:%.0f kgf", altitudeThr[i], velocityThr[j], maxThrAft[i][j]));
				}
			}
		} else {
			// radial inline
			// 获得增压器工作高度
//			VoidMeiMain.Companion.debugPrint("not a jet");
			aftbCoff = getdouble(hdrString + "Main.AfterburnerBoost");
//			VoidMeiMain.Companion.debugPrint(hdrString);
...
//				VoidMeiMain.Companion.debugPrint(String.format("*s%d*:[%.0f]%.0fhp - [%.0f]%.0fhp", i, compAlt[i], compPower[i] * compRpmRatio[i] * aftbCoff, compCeil[i], compCeilPwr[i] * compRpmRatio[i] * aftbCoff));
			}
        ...
        VoidMeiMainKt.getLogger().info("RPM Mult {}", engineRPMMultWEP);
		// 针对幻影2000C mode6 rpm乘数1.01的修复
		maxRPM = maxRPM * engineRPMMultWEP;
        VoidMeiMainKt.getLogger().info("RPM {}", maxRPM);
		maxAllowedRPM = getdouble("RPMMaxAllowed");
//		VoidMeiMain.Companion.debugPrint("RPM"+maxAllowedRPM);
		...
		// 	showEngineLoad(engLoad, i);
		// }
		// VoidMeiMain.Companion.debugPrint(engLoad[0].WorkTime/engLoad[i].RecoverTime);
		// avgEngRecoveryRate = avgEngRecoveryRate / (maxEngLoad - 1);
        ...
        VoidMeiMainKt.getLogger().info("Combat air weight: {}", emptyweight + oil + nitro);
...
//		}
        ...
        VoidMeiMainKt.getLogger().info("Flaps destruction num {}, gear {}", FlapsDestructionNum, GearDestructionIndSpeed);
		...
		/* 可变翼 */
		FullFlapsWing_V100 = new FMParts();
...
//		VoidMeiMain.Companion.debugPrint("机翼安装角" + WingAngle);
		...
		// VoidMeiMain.Companion.debugPrint(AWing * NoFlapsWing.ClCritHigh/)
		...
		// VoidMeiMain.Companion.debugPrint(NoFlapWLL+","+FullFlapWLL + ","+CdS);
...
//		
////		VoidMeiMain.Companion.debugPrint(String.format("Wx 250, 300, 350: %.0f, %.0f, %.0f", Wx250, Wx300, Wx350));
//		
//		VoidMeiMain.Companion.debugPrint("滚转效率 :" + 83.333f* Wx_vcoff);
//		VoidMeiMain.Companion.debugPrint("Wx_max :" + WxMax);
//		VoidMeiMain.Companion.debugPrint("Wx_600 :" + Wx600 +", aileronMax:" + ((600 - aileronEff) * aileronPowerLoss));
//		s += 		
...
//				+ "\n散热器/油冷器阻力系数: [" + RadiatorCd + ", " + OilRadiatorCd + "]\n";
		...
		fmdata = s;
//		VoidMeiMain.Companion.debugPrint(s);
		// VoidMeiMain.Companion.debugPrint("" + GearDestructionIndSpeed);
		// VoidMeiMain.Companion.debugPrint(wtload0+" "+oilload0);
		// VoidMeiMain.Companion.debugPrint(wtload1+" "+oilload1);
		// VoidMeiMain.Companion.debugPrint(wtload2+" "+oilload2);
		// VoidMeiMain.Companion.debugPrint(wtload3+" "+oilload3);
		// VoidMeiMain.Companion.debugPrint(wtload4+" "+oilload4);
		// VoidMeiMain.Companion.debugPrint(wtload5+" "+oilload5);
		// VoidMeiMain.Companion.debugPrint(subSt(getone("Load0")));
	}
	...
	public void transUnit() {
		...
		if (unitSystem.contains("Imperial")) {
			// VoidMeiMain.Companion.debugPrint("英制");
			...
			for (int i = 0; i < loc3.cur; i++) {
				...
				// VoidMeiMain.Companion.debugPrint(loc3.x[i]+" "+loc3.y[i]);
			}
	...
	public String getArray(String label) {
		...
		// VoidMeiMain.Companion.debugPrint(text);
		// 第二步获得值
		int bix = 0;
		int eix = 0;
	...
	public String getoneinData(String D, String label) {
		...
		label = label.substring(clsbix);
//		VoidMeiMain.Companion.debugPrint(label);
		// 第二步获得值
		int bix = 0;
		int eix = 0;
		bix = text.toUpperCase().indexOf(label.toUpperCase());
