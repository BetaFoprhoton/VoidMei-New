...
//		VoidMeiMain.debugPrint(String.format("total %d climb data logged", fA.curaltStage));
		bw.flush();
	}
	
	public void saveClimbData(){
		FileWriter tcsv = null;
//		VoidMeiMain.debugPrint("climbdata save to "+ climbName);
		try {
			tcsv = new FileWriter(climbName, true);
			// VoidMeiMain.debugPrint("打开文件成功");
		} catch (IOException e) {
...
//		VoidMeiMain.debugPrint(String.format("total %d roll data logged", k));
		bw.flush();
	}
	
	public void saveRollData(){
		FileWriter tcsv = null;
//		VoidMeiMain.debugPrint("rolldata save to "+ climbName);
		try {
	...
	void writeNyData(FileWriter txt) throws IOException {
		...
		for (int i = 0; i < FlightAnalyzer.maxIASStage; i++){
			...
			if (fA.turnLoad[i] > 0){
				...
				bw.write(fA.turnLoad[i]+", ");
				bw.write(fA.sepLoss[i] + "\n");
			}
//			bw.write("\n");
...
//		VoidMeiMain.debugPrint("rolldata save to "+ climbName);
	...
	public void init(Controller tc, Service s) {
		...
		xs = s;
		doit = false;
		//VoidMeiMain.debugPrint("flightlog初始化了");
...
//			csv = new FileWriter(fileName, true);
	...
	public void run() {
		...
		while (logon) {
			...
			//VoidMeiMain.debugPrint("flightlog内存溢出测试");
			// VoidMeiMain.debugPrint("执行");
			while (doit) {
				logTick();
				doit = false;// 写完后关闭
			}
