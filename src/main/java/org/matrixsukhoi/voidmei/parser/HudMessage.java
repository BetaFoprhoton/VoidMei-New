...
public class HudMessage {
	...
	public void init() {
		//VoidMeiMain.debugPrint("hudMSG初始化了");
		dmg = new damage();
	}

	public int update(String S, int lastDmg) {
		s = S;
		// VoidMeiMain.debugPrint(S);
		//String buf = getLine("damage");
		dmg.updated=false;
		String lastbuf = getDmglastLine();
		//VoidMeiMain.debugPrint(lastbuf);
		if (parseObj(lastbuf) == 1) {
			//VoidMeiMain.debugPrint(dmg.id + " " + dmg.msg);
			dmg.updated=true;
			return dmg.id;
		}
