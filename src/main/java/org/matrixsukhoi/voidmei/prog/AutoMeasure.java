...
public class AutoMeasure implements Runnable {
	...
	double findMaxSpd(double curSpd) throws IOException{
		...
		if (Math.abs(SEP) >= 1){
			...
			xS.httpClient.fmCmdSetSpd(curSpd, VoidMeiMain.requestDest);
		}
		return curSpd;
	}
	...
	public void run() {
		...
		while (doit) {
			...
			if (xS.sState.gear != 100 || (xS.speedv > 10 && xS.sState.throttle > 0)) {
				...
				try {
					if (start == 0){
						xS.httpClient.fmCmdSetAlt(3000, VoidMeiMain.requestDest);
						
						// spd = 100;
						xS.httpClient.fmCmdSetSpd(spd, VoidMeiMain.requestDest);
						start = 1;	
					} else {
						// Thread.sleep(5000);
						// spd = findMaxSpd(spd);
