...
public class FlapsControl implements Runnable {
	...
	public void run() {
		...
		// VoidMeiMain.debugPrint(flaps+" "+isRun);
		while (isRun) {
			...
			updateflaps();
			// VoidMeiMain.debugPrint(flaps);
			// 自动襟翼
			/*
			 * if (flaps > 0) { //VoidMeiMain.debugPrint(spd+" "+objflaps); if
			 * (flaps > objflaps&&isDown) { //VoidMeiMain.debugPrint("襟翼收回");
			 ...
			 * //VoidMeiMain.debugPrint("襟翼放下"); A.keyPress(KeyEvent.VK_TAB); try {
