...
import static org.matrixsukhoi.voidmei.parser.StringHelperKt.F_INVALID;
...
public class Service implements Runnable {
	...
	public void trans2String() {
		...
		// // VoidMeiMain.debugPrint(iIndic.engine_temperature);
		...
		if (hasWingSweepVario) {
			sWingSweep = String.format("%.0f", sIndic.wSweepIndicator * 100.f);
			// VoidMeiMain.debugPrint(sWingSweep);
		} else
			sWingSweep = nastring;
		if (sIndic.radioAltitude >= 0) {
		...
		sPitchUp = String.format("%.0f", sIndic.aviaHorizonPitch);
		...
		// VoidMeiMain.debugPrint("已加力时间(秒)"+wepTime/1000);
		// VoidMeiMain.debugPrint("剩余加力时间(分钟)"+rwepTime);
			...
			if (Math.abs(checkEngineType) >= 100) {
				...
				if (checkEngineType >= 0) {
					...
					if (checkPitch > 0) {
						...
						// VoidMeiMain.debugPrint("涡桨\n");
					} else
						iEngType = ENGINE_TYPE_JET;
				}

				// VoidMeiMain.debugPrint(String.format("自适应判断引擎类型 %d\n", iEngType));
			}
		}
	}
	...
	public void slowcalculate(long dtime) {
		// 计算耗油率及持续时间
		// VoidMeiMain.debugPrint(totalfuelp - totalfuel);
		...
		if (dfuel > 0) {
			...
			if (!bLowAccFuel) {
				...
				if (fueltime > 0)
					...
					fueltime = tmpft;
				// VoidMeiMain.debugPrint("" + fueltime +" " + tmpft);
			}
			// VoidMeiMain.debugPrint(fuelChange);
		...
		// 计算变化率
		// VoidMeiMain.debugPrint("" + fueltime);
	}

	// public engineLoad[] sPL;
	public void checkOverheat() {
		Blkx.engineLoad[] pL = c.blkx.engLoad;
		// curLoad = c.Blkx.findmaxLoad(pL, nwaterTemp, noilTemp);
		// 减去时间
		double minWorkTime = 99999 * 1000;
		/* 关发动机后，温度降到最低load后恢复 */
		boolean engOff = sState.power[0] == 0 && sState.throttle > 0;
        /* 关发动机 */
        // VoidMeiMain.debugPrint("监测到引擎关闭");
        // 水冷
			...
			if (i < curWLoad) {
				...
				if (engOff) {
					...
					if (curWLoad == 0 || pL[curWLoad - 1].WorkTime < 0.1) {
						// VoidMeiMain.debugPrint("回复水温耐久条");
		...
		// VoidMeiMain.debugPrint("当前水工作负载: " + curLoad + "," + minWorkTime);
		// VoidMeiMain.debugPrint("水工作负载数组: [");
		// for (int i = 0; i < c.Blkx.maxEngLoad; i++) {
		// System.out.print(pL[i].curWaterWorkTimeMili / 1000 + " ");
		// }
		// VoidMeiMain.debugPrint("]");
			...
			if (i < curOLoad) {
				...
				if (engOff) {
					...
					if (curOLoad == 0 || pL[curOLoad - 1].WorkTime < 0.1) {
						// VoidMeiMain.debugPrint("回复油温耐久条");
		...
		//// VoidMeiMain.debugPrint("当前油工作负载: " + curLoad + "," + minWorkTime);
		// VoidMeiMain.debugPrint("油工作负载数组: [");
		// for (int i = 0; i < c.Blkx.maxEngLoad; i++) {
	...
	public void calculateB() {
		...
		// VoidMeiMain.debugPrint(bangleR +"," + bxoffset);
	...
	public void updateWepTime() {
		...
		for (int i = 0; i < engineNum; i++) {
			if (sState.throttles[i] > 100) {
				// 进入Wep状态
				// VoidMeiMain.debugPrint(TimeIncrMili);
	...
	public void updateAlt() {
		...
		// if (checkAlt > 2)
		// alt = alt * 0.3048f;
		// // VoidMeiMain.debugPrint(Math.abs(alt - altp)*1000+"?"+Math.abs(2 *
		// // sState.Vy * intv));
		//
		// // 解决熊猫的高度问题
		// alt = alt + altperCircle * altreg;
		// VoidMeiMain.debugPrint("checkalt"+checkAlt);

		// 无线电高度
		pRadioAlt = radioAlt;
		// radioAlt = iIndic.radio_altitude;

		if (sIndic.radioAltitude == F_INVALID) {
			radioAlt = alt;
			radioAltValid = false;
		} else {
			radioAltValid = true;
			if (iCheckAlt > 0) {
				radioAlt = sIndic.radioAltitude * 0.3048f;
			} else {
				radioAlt = sIndic.radioAltitude;
			}
		}
		dRadioAlt = (ratio_1 * dRadioAlt) + ratio * 1000.0f * (radioAlt - pRadioAlt) / intv;
		// VoidMeiMain.debugPrint(dRadioAlt);
	...
	public void updateTurn() {
		...
		if (sIndic.aviaHorizonRoll != -65535 && sIndic.aviaHorizonPitch != -65535) {
			// 获得横滚角

			An = (double) (g * Math
					.sqrt(sState.Ny * sState.Ny + 1 - 2 * sState.Ny * Math.cos(Math.toRadians(sIndic.aviaHorizonRoll))
							* Math.cos(Math.toRadians(sIndic.aviaHorizonPitch + sState.AoA))));
			// An = (double)(g * Math.sqrt(a))
		} else
			An = (g * sState.Ny);
		// VoidMeiMain.debugPrint(Math.cos(sIndic.aviahorizon_roll));
		// 计算时取前后两次采样的速度平均值
	...
	public void updateSpeed() {
		...
		// VoidMeiMain.debugPrint("校正TAS:"+ speedv*3.6);
		// 订正后加速度还是会有跳变
		// VoidMeiMain.debugPrint("校正TAS:"+ speedv*3.6 + "," + iastotascoff);
	...
	public void updateEngineState() {
		...
		if (!isEngJet()) {
			...
			for (i = 0; i < engineNum; i++) {
				...
				// VoidMeiMain.debugPrint(sState.engineNum);
				ttotalhp = ttotalhp + sState.power[i];
				ttotalhpeff = ttotalhpeff + sState.thrust[i] * g * speedv / 735;
			}
			// System.out.println(ttotalhp);
			// VoidMeiMain.debugPrint(totalhpeff);
			// VoidMeiMain.debugPrint(String.format("sevice 引擎数量%d, 功率%.0f", engineNum, ttotalhp));
			...
			double ttotalthr = 0;
			for (i = 0; i < engineNum; i++) {
				// VoidMeiMain.debugPrint(sState.thrust[0]);
				ttotalthr = ttotalthr + sState.thrust[i];
			}
			// VoidMeiMain.debugPrint(totalthr+" "+totalhpeff);
	...
	public void updateFuel() {
		...
		// VoidMeiMain.debugPrint("I"+totalfuel);
		if (fTotalFuel == 0) {
	...
	public void updateSEP() {
		...
		// VoidMeiMain.debugPrint(diffspeed);
	...
	public void checkWing() {
		if (sIndic.wSweepIndicator != -65535)
	...
	public void getMaximumRPM() {
		if (!getMaximumRPM) {
			if (c.blkx != null && c.blkx.valid) {
				...
				// VoidMeiMain.debugPrint(maximumThrRPM);
				getMaximumRPM = true;
			} else {
				// 自适应获得(无FM)
				...
				if (checkMaxiumRPM < 20000 / freq) {
					if (sState.IAS > 50) {
						if (sState.RPM >= maximumThrRPM) {
							// VoidMeiMain.debugPrint(sState.RPM
							// +","+maximumThrRPM);
	...
	public void calculate() {
		...
		// VoidMeiMain.debugPrint(horizontalLoad);
		// 计算总推力、总功率和总实功率
		updateEngineState();
		// 计算总油量
		updateFuel();
		...
		// 襟翼判断
		checkFlap();
		// VoidMeiMain.debugPrint(flapAllowSpeed);
	...
	public double getFlapAllowSpeed(int flapPercent, Boolean isDowningFlap) {
		...
		if (i == -1) {
			...
			if (isDowningFlap && FlapsDestructionNum >= 1) {
				...
				// // VoidMeiMain.debugPrint(x0 + "-" + x1 + ", " + y0 + "-" + y1 + " k " + k + " : F" + flapPercent + "D "
				// // + (flapPercent - x0) * k + " L" + (y0 + (flapPercent - x0) * k));
				// VoidMeiMain.debugPrint("limit " + ( y0 + (flapPercent - x0) * k));
			...
			// 速度等于
			// VoidMeiMain.debugPrint(x0 + "-" + x1 + ", " + y0 + "-" + y1);
			return y0 + (flapPercent - x0) * k;
		}
	...
	public double getFlapAllowAngle(double ias, Boolean isDowningFlap) {
		...
		if (i == -1) {
			// VoidMeiMain.debugPrint(c.Blkx.FlapsDestructionIndSpeed[i + 1][1] + " spd " + ias);
			return 0;
		}
		...
		k = this.calcK(x0, y0, x1, y1);
		// 速度等于
		// VoidMeiMain.debugPrint("[" +i + "]" + "ias" + ias + "limit" + x0 + "-" + x1 + ", " + y0 + "-" + y1);
		t = y0 + (ias - x0) * k;
	...
	public void resetvaria() {
		...
		calcSpeedSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		diffSpeedSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		sepSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		turnrdsSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		sumSpeedSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		energyDiffSMA = new CalcHelper.SimpleMovingAverage((int) (1000 / freq));
		fuelTimeSMA = new CalcHelper.SimpleMovingAverage(4);
		if (c.blkx != null) {
			Blkx.engineLoad[] pL = c.blkx.engLoad;
	...
	public void checkState() {
		...
		// VoidMeiMain.debugPrint("s:"+httpClient.strState+"s1:"+httpClient.strIndic);
		// 更新state

		c.initStatusBar();
		if (httpClient.strState.length() > 0 && httpClient.strIndic.length() > 0) {
			// 改变状态为连接成功
			// VoidMeiMain.debugPrint(sState);
			...
			if (sState.flag && sIndic.flag) {
				...
				if ((!sIndic.type.equals("DUMMY_PLANE")) && ((sState.totalThr != 0) || (sState.RPM != 0))) {
					if (playerLive == false) {
						if (!portOcupied)
							httpClient.getReqMapInfoResult(VoidMeiMain.requestDest);
						else
							httpClient.getReqMapInfoResult(VoidMeiMain.requestDestBkp);
						mapinfo.update(httpClient.strMapInfo);
					}
					playerLive = true;
					// VoidMeiMain.debugPrint("grid_zero: " + mapinfo.grid_zeroX + ", " +
					// mapinfo.grid_zeroY);
				}
				...
				if (isPlayerLive()) {
					...
					if (!c.cur_fmtype.equals(sIndic.type)) {
						// 机型变化
						VoidMeiMainKt.getLogger().info("Model has changed, restarting the program.");
						c.S4toS1();
					}
					// speedvp = sState.IAS;
					// 开始计算数据
					...
					if ((Math.abs(speedv) < 10) && (fTotalFuel - fTotalFuelP > 1)) {
						// VoidMeiMain.debugPrint("检测到油量增加 " + fTotalFuel + "," + fTotalFuelP);
						VoidMeiMainKt.getLogger().info("Refuel and reset variables.");
					...
					if (sState.totalThr == 0 && sState.RPM <= 0 && sState.IAS < 10) {
						VoidMeiMainKt.getLogger().info("Detected player crash.");
						playerLive = false;
					}
				}
			} else {
				...
				c.S4toS1();
				// VoidMeiMain.debugPrint("等待游戏开始");
				try {
					Thread.sleep(500);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					VoidMeiMainKt.getLogger().error("Interrupted Exception occur: {}", e.getMessage());
				}
			}
			...
			conState = -1;
			c.S4toS1();
			VoidMeiMainKt.getLogger().info("Waiting for connection...");
		}
		if (conState == -1) {
			// 端口连接可能有问题，切换端口
			// VoidMeiMain.debugPrint("切换端口\n");
	...
	@Override
	public void run() {
		long waitMili = VoidMeiMain.threadSleepTime;
		// VoidMeiMain.debugPrint("" + waitMili);
		while (true) {
			...
			if (diffTime >= freq) {
				...
				// 尝试GET数据
				if (!portOcupied)
					httpClient.getReqResult(VoidMeiMain.requestDest);
				else
					httpClient.getReqResult(VoidMeiMain.requestDestBkp);
				...
				// VoidMeiMain.debugPrint("deadline Miss, try catch\n" + SystemTime +
				// "," + MainCheckMili);
				// }
			}
			...
			if (diffTime1 >= 10 * freq) {
				...
				if (!portOcupied)
					httpClient.getReqMapObjResult(VoidMeiMain.requestDest);
				else
					httpClient.getReqMapObjResult(VoidMeiMain.requestDestBkp);
