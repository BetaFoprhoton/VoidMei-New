...
public class AttitudeIndicator extends WebFrame implements Runnable {
	...
	int[] pX;
	int[] pY;
	Color transParentWhite = VoidMeiMain.colorUnit;
	...
	public void locater(Graphics2D g2d, int width, int height, int x, int y, int pitch, int center_round,
		...
		for (int i = 0; i < 2 * tickLine; i++) {
			...
			// VoidMeiMain.debugPrint("draw" + (4 + 2 * i));
		}
		...
		g2d.setColor(VoidMeiMain.colorUnit);
	...
	public void init(Controller c, Service s) {
		...
		if (xc.getconfig("attitudeIndicatorUseNumColor") != "")
			if (Boolean.parseBoolean(xc.getconfig("attitudeIndicatorUseNumColor")))
				transParentWhite = VoidMeiMain.colorNum;
...
//		setFocusableWindowState(false);// 取消窗口焦点
	...
	public void drawTick() {
		...
		Pitch = Math.round((-xs.sIndic.aviaHorizonPitch + MaxAoA) * xHeight / (2 * MaxAoA));
		...
		// 使用剩余
		// VoidMeiMain.debugPrint(1/2 * xc.Blkx.aoaHigh);
		if (xc.blkx.valid) {
			if (showAoALimits) {
				if (xs.sState.AoA >= 0) {
		...
		for (int i = 0; i < tickLine; i++) {
			...
			// VoidMeiMain.debugPrint("pS" + (4 + 4 * i) + "角度" + (start
			// +
			// (dTick * (i + 1))) + ","
			// + (-start - (dTick * (i + 1))));
			// VoidMeiMain.debugPrint((int) Math.round((start + (dTick *
			// (i
			// + 1))) / MaxAoA * xHeight) + ","
		...
		rotatePointMatrix(pS, xs.sIndic.aviaHorizonRoll, pC, pT, pS.length);
		...
		// VoidMeiMain.debugPrint(AoA + "," + AoS +":["+xs.AoA
		// +","+xs.AoS+"]");
