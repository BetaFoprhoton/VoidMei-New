...
public class DrawFrameImpl extends WebFrame implements Runnable {
	/**
	 * 
	 */
	...
	void paintAction(Graphics g, Blkx fmblk) {
		...
		// VoidMeiMain.debugPrint(xmin+" "+xmax);
...
//		xmax对齐10
	...
	void drawXY(Graphics2D g, int x, int y, int dwidth, int dheight, String title, String xName, String yName,
		...
		// 标题
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, fontsize+6));
		g.drawString(title, x + dwidth / 2, y);
		y = y + 10;// 往下推10
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, fontsize));
		...
		// x轴单位
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, fontsize+4));
	...
	void drawPoint(Graphics2D g, int x, int y, int dwidth, int dheight, double ggx, double ggy, double ix[], double iy[],
		...
		for (ii = 0; ii < ix.length; ii++) {
			// VoidMeiMain.debugPrint((y + dheight) +" "+(y + dheight
			// -(iy[ii]-pymin) * ggy));
	...
	void drawCoordinates(Graphics2D g, int x, int y, int dwidth, int dheight, String title, String xName, String yName,
		...
		// 标题
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, 16));
		g.drawString(title, x + dwidth / 2, y);
		y = y + 10;// 往下推10
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, 10));
		// x轴与箭头
		g.drawLine(x, y + dheight, x + dwidth, y + dheight);
		...
		for (; ii >= 0; ii--) {
			...
			g.drawLine((int) (movex + x + ii * intervalX * ggx), y + dheight, (int) (movex + x + ii * intervalX * ggx),
					y);
			// VoidMeiMain.debugPrint("X坐标"+(pmin + ii * intervalX)+"位置"+(int)
			// (movex + x + ii * intervalX * ggx));
			g.drawString(String.valueOf(pmin + ii * intervalX), (int) (movex + x + ii * intervalX * ggx),
					y + dheight + 15);
		}
		// x轴单位
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, 14));
		g.drawString(xD, x + dwidth + 5, y + dheight);
		g.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, 10));
