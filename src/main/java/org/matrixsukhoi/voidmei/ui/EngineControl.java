...
public class EngineControl extends WebFrame implements Runnable {
	...
	Color lblShadeColor = VoidMeiMain.colorShade;
	...
	public void drawTPMRC(Graphics2D g2d, int x, int y) {
		...
		for (int i = 0; i < leftUseNum; i++) {
			...
			if (i == lidx_r || i == lidx_c || i == lidx_m || i == lidx_f) {
				...
				// 横着画
				// if(isJet) continue;
				UIBaseElem.drawHBarTextNum(g2d, x, y + dy, 4 * fontsize, fontsize >> 1, leftValPix[i], 1, VoidMeiMain.colorNum,
						leftLblNum[i][0], leftLblNum[i][0] + leftLblNum[i][1], fontLabel, fontLabel);
				dy += 1 * fontsize + (fontsize >> 2);
			} else {
				UIBaseElem.drawVBarTextNum(g2d, x + dx, y, fontsize >> 1, 4 * fontsize, leftValPix[i], 1, VoidMeiMain.colorNum,
						leftLblNum[i][0], leftLblNum[i][0] + leftLblNum[i][1], fontLabel, fontLabel);
				dx += (5 * fontsize) >> 1;
			}
