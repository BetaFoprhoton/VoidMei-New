...
public class FlightInfo extends WebFrame implements Runnable {
	...
	Color lblColor = VoidMeiMain.colorUnit;
	Color lblNameColor = VoidMeiMain.colorLabel;
	Color lblNumColor = VoidMeiMain.colorNum;
	...
	// String s, Font f) {
	// __drawStringShade(g2d, x, y, shadeWidth, s, f, VoidMeiMain.lblNumColor);
	// }
	//
	// // 横向还是竖向
	...
	// sNum, num, VoidMeiMain.lblNumColor);
	//
	// // 标签名
	// __drawStringShade(g2d, x_offset + 3 * num.getSize(), y_offset,
	// shadeWidth, sLabel, label, VoidMeiMain.lblNameColor);
	// // 单位名
	// __drawStringShade(g2d, x_offset + 3 * num.getSize(), y_offset +
	// label.getSize(), shadeWidth, sUnit, unit,
	// VoidMeiMain.lblColor);
	// }
	...
	public void webLabelRemoveML(WebLabel label) {
		...
		for (int i = 0; i < tmp.length; i++) {
			// VoidMeiMain.debugPrint("remove compnents" + tmp[i] +
			// "mouseListener");
	...
	public void webPanelRemoveML(WebPanel label) {
		...
		for (int j = 0; j < mls.length; j++) {
			// VoidMeiMain.debugPrint("remove ml" + mls[j]);
			label.removeMouseListener(mls[j]);
		}
		for (int j = 0; j < mmls.length; j++) {
			// VoidMeiMain.debugPrint("remove mls" + mmls[j]);
...
//		totalString[idx_rds][2] = "m"+xs.sN;
	...
	public void init(Controller c, Service s) {
		...
		// VoidMeiMain.debugPrint(useNum / columnNum + addnum + 1);
...
//		setFocusable(false);
//
//		// VoidMeiMain.debugPrint(this.isAlwaysOnTopSupported());
//		// setAlwaysOnTop(true);
