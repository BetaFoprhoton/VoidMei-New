...
public class GearAndFlaps extends WebFrame implements Runnable {
	...
	Color transParentWhite = VoidMeiMain.colorNum;
	Color transParentWhitePlus = VoidMeiMain.colorNum;
	Color warning = VoidMeiMain.colorWarning;
	...
	public void initPreview(Controller xc) {
		init(xc, null);
		// VoidMeiMain.debugPrint("初始化");
		// setShadeWidth(10);
		this.setVisible(false);
		// this.getWebRootPaneUI().setTopBg(new Color(0, 0, 0, 50));
		this.getWebRootPaneUI().setMiddleBg(VoidMeiMain.previewColor);// 中部透明
		this.getWebRootPaneUI().setTopBg(VoidMeiMain.previewColor);// 顶部透明
	...
	public WebLabel createWebLabel(String text) {
		...
		l1.setShadeColor(VoidMeiMain.colorShade);
		l1.setDrawShade(true);
		l1.setForeground(VoidMeiMain.colorLabel);
		l1.setFont(new Font(VoidMeiMain.defaultFontName, Font.PLAIN, 12));
		return l1;
	}
	...
	public void initpanel() {
		 ...
		 * Font(VoidMeiMain.DefaultFontName,Font.PLAIN,12));
		...
		// s1.setFont(new Font(VoidMeiMain.DefaultNumfontName,Font.PLAIN,10));

		s1.setBounds(0, 0, 90, 30);
		s1.setFontSize(10);
		s1.setForeground(VoidMeiMain.colorLabel);
		s1.setDrawShade(true);
		s1.setShadeColor(VoidMeiMain.colorShade);
		s1.setBottomBgColor(new Color(0, 0, 0, 0));
		s1.setTopBgColor(new Color(0, 0, 0, 0));
		s1.setFont(VoidMeiMain.defaultFont);
		// s1.setSelected(true);
	...
	public void init(Controller c, Service s) {
		...
		warnColor = VoidMeiMain.colorNum;
		...
		panel = new WebPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				// dy+=1.5 * fontSize;
				dy += barHeight + gap;
				UIBaseElem.drawVBarTextNum(g2d, 0, dy, barWidth, barHeight, flapPix, 1, VoidMeiMain.colorNum, "",
	...
	public void drawTick() {
		...
		if (xs.sState.gear >= 0) {
			if (xs.sState.gear == 0) {
				...
				// warnColor = VoidMeiMain.colorWarning;
				// // VoidMeiMain.debugPrint(xs.sState.airbrake);
				// } else {
				warnText = "";
				warnColor = VoidMeiMain.colorNum;
				// VoidMeiMain.debugPrint(xs.sState.airbrake);
		...
		// // VoidMeiMain.debugPrint("gearandFlaps执行了");
