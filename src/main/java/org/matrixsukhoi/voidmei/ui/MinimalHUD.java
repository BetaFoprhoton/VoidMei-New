...
import org.matrixsukhoi.voidmei.prog.OtherService;
...
public class MinimalHUD extends WebFrame implements Runnable {
	...
	public void drawBlinkX(Graphics2D g) {
		...
		if (blinkX) {
			if (!blinkActing) {
				...
				g.setColor(VoidMeiMain.colorShadeShape);

				g.drawLine(2, 2, Width - 2, Height - 2);
				g.drawLine(Width - 2, 2, 2, Height - 2);
				g.setStroke(new BasicStroke(3));
				g.setColor(VoidMeiMain.colorNum);
			...
			if (blinkCheckTicks % blinkTicks == 0) {
				// VoidMeiMain.debugPrint(blinkTicks +"?" + blinkCheckTicks);
	...
	public void drawTextseries(Graphics2D g, int x, int y) {
		...
		UIBaseElem.drawVBarTextNumLeft(g, kx + barWidth, n5 + lineWidth + 2, barWidth, throttley_max, throttley, 1,
				VoidMeiMain.colorNum, throttleColor, "",
		...
		if (drawAttitude && !disableAttitude) {
			if (!blinkX || (blinkX && !blinkActing)) {
				...
				g.setStroke(Bs1);
				g.setColor(VoidMeiMain.colorLabel);
				g.drawLine(circleX + aosX, circleY + pitch, circleX, circleY);
				...
				// 画文字
				if (roundHorizon >= 0) {
					UIBaseElem.__drawStringShade(g, circleX, circleY - 1, 1, sAttitude, drawFontSmall, VoidMeiMain.colorNum);
				} else {
					UIBaseElem.__drawStringShade(g, circleX, circleY - 1, 1, sAttitude, drawFontSmall, VoidMeiMain.colorUnit);
				}
				// UIBaseElem.__drawStringShade(g, circleX - HUDFontsize / 2, circleY + 3 *
				// HUDFontsize / 2, 1, sAttitudeRoll, drawFontSmall, VoidMeiMain.colorNum);
		...
		for (int i = 0; i < 5; i++) {
			...
			if ((i == 2 && inAction) || (i == 0 && warnVne) || (i == 1 && warnRH)) {
				UIBaseElem.__drawStringShade(g, x, n + y, 1, lines[i], drawFont, VoidMeiMain.colorWarning);
			} else
				UIBaseElem.__drawStringShade(g, x, n + y, 1, lines[i], drawFont, VoidMeiMain.colorNum);
		...
		// g.setColor(VoidMeiMain.lblShadeColorMinor);
		// g.drawLine(3, n, 3, n - throttley);
		// g.setColor(VoidMeiMain.lblNumColor);
		// g.drawLine(2, n - 1, 2, n - 1 - throttley);

		// UIBaseElem.drawVRect(g, kx, n, barWidth, aoaY, 1, VoidMeiMain.warning);
		...
		// throttley, 1, VoidMeiMain.lblNumColor, "", "T"+throttley, drawFont,
		// drawFont);
		// UIBaseElem.drawVBar(g, kx, n, barWidth, HUDFontsize * 5, throttley,
		// 1, VoidMeiMain.lblNumColor);
		if (!drawAttitude) {
			/*
			 * // if (pitch > 0) { // UIBaseElem.drawVRect(g, kx, n, barWidth,
			 * pitch, 1, // VoidMeiMain.colorWarning); // } else { //
			 * UIBaseElem.drawVRect(g, kx, n, barWidth, pitch, 1, VoidMeiMain.colorNum);
			 * // } // kx += barWidth;
			 */ }

		// UIBaseElem.drawVBar(g, x + rightDraw, n, barWidth, AoAFuselagePix,
		// aoaY, 1, VoidMeiMain.warning);
		// AoA
		// g.setColor(VoidMeiMain.lblShadeColorMinor);
		// g.drawLine(1, n, 1, n - aoaY);
		// g.setColor(VoidMeiMain.warning);
		...
		// g.setColor(VoidMeiMain.warning);
		// if (pitch > 0){
		// UIBaseElem.drawVRect(g, rightDraw, n, 5, pitch, 1, VoidMeiMain.lblNumColor);
		// }
		// else{
		// UIBaseElem.drawVRect(g, rightDraw, n, 5, pitch, 1, VoidMeiMain.warning);
		// }
	...
	public void initPreview(Controller controller) {
		init(controller, null, null);
		// setShadeWidth(10);
		this.setVisible(false);
		this.getWebRootPaneUI().setTopBg(VoidMeiMain.previewColor);
		this.getWebRootPaneUI().setMiddleBg(VoidMeiMain.previewColor);
	...
	public void init(Controller c, Service s, OtherService os) {
		...
		if (!xc.getconfig("MonoNumFont").isEmpty())
			NumFont = xc.getconfig("MonoNumFont");
		else
			NumFont = VoidMeiMain.defaultNumfontName;
		if (!xc.getconfig("crosshairX").isEmpty())
			lx = Integer.parseInt(xc.getconfig("crosshairX"));
		else
			lx = (VoidMeiMain.screenWidth - Width) / 2;
		if (!xc.getconfig("crosshairY").isEmpty())
			ly = Integer.parseInt(xc.getconfig("crosshairY"));
		else
			ly = (VoidMeiMain.screenHeight - Height) / 2;
		if (!xc.getconfig("crosshairScale").isEmpty())
			CrossWidth = Integer.parseInt(xc.getconfig("crosshairScale"));
		else
			CrossWidth = 70;
		if (CrossWidth == 0)
			CrossWidth = 1;
		if (!xc.getconfig("crosshairName").isEmpty())
			crosshairName = xc.getconfig("crosshairName");
		else
			crosshairName = "";
		// VoidMeiMain.debugPrint(xc.getconfig("usetexturecrosshair"));
		if (!xc.getconfig("displayCrosshair").isEmpty()) {
		...
		aoaY = 10;
		disableAoA = false;
		throttleColor = VoidMeiMain.colorShadeShape;
		lineAoA = String.format("α%3.0f", 20.0);
		relEnergy = "E114514";
		sAttitude = "";
		sAttitudeRoll = "";
		if (!xc.getconfig("disableHUDAoA").isEmpty()) {
		...
		if (aoaY > rightDraw)
			aoaY = rightDraw;
		aoaColor = VoidMeiMain.colorNum;
		aoaBarColor = VoidMeiMain.colorNum;
		...
		// VoidMeiMain.debugPrint(lx);
		// VoidMeiMain.debugPrint(ly);
		...
		panel = new WebPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				// g2d.draw
				g2d.setPaintMode();
				g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, VoidMeiMain.graphAASetting);
				g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, VoidMeiMain.textAASetting);
		...
		//
		this.add(panel);
		// if (VoidMeiMain.debug)setShadeWidth(8);
		...
		// this.setCursor(VoidMeiMain.blankCursor);
		// setVisible(true);
		// 1miao 8 ci
	...
	public void updateString() {
		...
		if (throttle > 101) {
			throttleColor = VoidMeiMain.colorWarning;
		} else {
			throttleColor = VoidMeiMain.colorNum;
		}
		...
		OilX = OilX - 360;
		// OilX = OilX - 180;
		double aviahp = xs.sIndic.aviaHorizonPitch;
		double aviar = xs.sIndic.aviaHorizonRoll;
		...
		if (aviahp != -65535)
			...
			pitch = 0;
		// VoidMeiMain.debugPrint(-(aviahp+aoa));
		...
		if (xc.blkx != null && xc.blkx.valid) {
			...
			double vwing = 0;
			if (xc.blkx.isVWing) {
				vwing = xs.sIndic.wSweepIndicator;
			}
			...
			if (availableAoA < aoaWarningRatio * maxAvailableAoA) {
				aoaColor = VoidMeiMain.colorWarning;
			} else {
				aoaColor = VoidMeiMain.colorNum;
			}
			if (availableAoA < aoaBarWarningRatio * maxAvailableAoA) {
				aoaBarColor = VoidMeiMain.colorUnit;
			} else {
				aoaBarColor = VoidMeiMain.colorNum;
			}
