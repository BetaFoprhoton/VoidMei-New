...
public class StickValue extends WebFrame implements Runnable {
	...
	int px;
	int py;
	static Color lblColor = VoidMeiMain.colorUnit;
	static Color lblNameColor = VoidMeiMain.colorLabel;
	static Color lblNumColor = VoidMeiMain.colorNum;
	...
	public void locater(Graphics2D g2d, int x, int y, int r, int width, int stroke) {
		...
		g2d.setColor(VoidMeiMain.colorShadeShape);
...
//		g2d.setColor(VoidMeiMain.lblShadeColorMinor);
		...
		g2d.setColor(VoidMeiMain.colorNum);
	...
	public void initslider(WebSlider slider1) {
		...
		slider1.setThumbBgBottom(VoidMeiMain.colorNum);
		slider1.setThumbBgTop(VoidMeiMain.colorNum);
...
//				
//				int dy = fontSize >> 1;
//				UIWebLafSetting.__drawLabelBOSType(g2d, width, dy, 1, fontNum, fontLabel, fontUnit, sElevator, sElevatorLabel, sElevatorUnit,9);
//				dy+=1.5 * fontSize;
//				UIWebLafSetting.__drawLabelBOSType(g2d, width, dy, 1, fontNum, fontLabel, fontUnit, sAileron, sAileronLabel, sAileronUnit,9);
//				dy+=1.5 * fontSize;
//				UIWebLafSetting.__drawLabelBOSType(g2d, width, dy, 1, fontNum, fontLabel, fontUnit, sRudder, sRudderLabel, sRudderUnit,9);
//				dy+=1.5 * fontSize;
//				UIWebLafSetting.__drawLabelBOSType(g2d, width, dy, 1, fontNum, fontLabel, fontUnit, sWingSweep, sWingSweepLabel, sWingSweepUnit,9);
//				
//				// 绘制横条
////				UIWebLafSetting.drawHBarTextNum(g2d, x, y, width, height, val_width, borderwidth, c, lbl, num, lblFont, numFont);
//				UIWebLafSetting.drawHBarTextNum(g2d, 0, height, width, fontSize >> 1, rudderValPix, 1, VoidMeiMain.lblNumColor, sRudderLabel, sRudder, fontLabel, fontLabel);
//				
//			}
//		};
...
//		lblX.setFont(new Font(VoidMeiMain.DefaultFontName, Font.BOLD, 12));
...
//		lblY.setFont(new Font(VoidMeiMain.DefaultFontName, Font.BOLD, 12));
...
//		lblW.setFont(new Font(VoidMeiMain.DefaultFontName, Font.BOLD, 12));
...
//		lblZ.setFont(new Font(VoidMeiMain.DefaultFontName, Font.BOLD, 12));
...
//	}
	...
	public void init(Controller c, Service s) {
		...
		xc = c;
		xs = s;
		// VoidMeiMain.debugPrint("stickValue初始化了");
		...
		if (xc.getconfig("flightInfoFontC") != "")
			...
			FontName = VoidMeiMain.defaultFont.getFontName();
		...
		topPanel =  new WebPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				// 绘制横条
//				UIWebLafSetting.drawHBarTextNum(g2d, x, y, width, height, val_width, borderwidth, c, lbl, num, lblFont, numFont);
				UIBaseElem.drawHBarTextNum(g2d, 0, height, width, fontSize >> 1, rudderValPix, 1, VoidMeiMain.colorNum, sRudderLabel, sRudder, fontLabel, fontLabel);
...
//			slider.setValue(xs.sState.rudder);
			...
			// VoidMeiMain.debugPrint("stickValue执行了");
